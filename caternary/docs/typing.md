...
- **The exploratory verdict is cacheable.** It is a property of the obligation, stable while the obligation and its in-scope facts are unchanged — memoize on the obligation's content-hash (the same per-obligation cache as the whole-program reverify story, architecture section). A warm compile pays it zero times.
