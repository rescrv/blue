...
mod tests {
    ...
    fn one_element_ranges() {
        assert_eq!(1, binary_search_by(1, 1, |_probe| Ordering::Less));
        assert_eq!(2, binary_search_by(2, 2, |_probe| Ordering::Less));
    }

    #[test]
    fn partitioning() {
        let slice = &[0, 1, 1, 1, 1, 2, 3, 5];
        assert_eq!(0, partition_by(0, 7, |x| slice[x] < 0));
        assert_eq!(1, partition_by(0, 7, |x| slice[x] < 1));
        assert_eq!(5, partition_by(0, 7, |x| slice[x] < 2));
        assert_eq!(6, partition_by(0, 7, |x| slice[x] <= 2));
        assert_eq!(7, partition_by(0, 7, |x| slice[x] <= 5));
    }
