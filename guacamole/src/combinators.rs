...
pub fn shuffle<'a, T: Clone>(values: &'a [T]) -> impl FnMut(&mut Guacamole) -> Vec<T> + 'a {
    move |guac| {
        ...
        for i in (1..values.len()).rev() {
            let j = <usize as RangeTo>::multiply(usize::from_guacamole(&mut (), guac), i + 1);
            values.swap(i, j);
        }
        values
    }
...
fn format_uuid(id: &[u8; 16]) -> String {
    ...
    for &group in &GROUPS {
        ...
        for _ in 0..group {
            ...
            out[dst + 1] = HEX[(byte & 0xf) as usize];
            src += 1;
            dst += 2;
        }
    }
    // `out` is assembled exclusively from ASCII hex digits and hyphens.
...
pub fn to_charset(chars: &str) -> impl FnMut(&[u8]) -> String {
    ...
    enum CharsetTable {
        Ascii(Box<[u8; 256]>),
        Unicode(Box<[char; 256]>),
    }

    let table = if s.iter().all(|ch| ch.is_ascii()) {
        ...
        CharsetTable::Ascii(Box::new(table))
    } else {
        ...
        CharsetTable::Unicode(Box::new(table))
    };

    move |bytes: &[u8]| match &table {
        CharsetTable::Ascii(table) => {
            let mut string = Vec::with_capacity(bytes.len());
            for b in bytes.iter() {
                string.push(table[*b as usize]);
            }
            // The table is built only when every source character is ASCII.
            unsafe { String::from_utf8_unchecked(string) }
        }
        CharsetTable::Unicode(table) => {
