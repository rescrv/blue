...
impl Coordination<TestState<'_>> for TestCoordination {
    fn acquire<'a: 'b, 'b>(mut guard: MutexGuard<'a, Self>, t: &'b mut TestState<'_>) -> (bool, MutexGuard<'a, Self>) {
    ...
    fn release<'a: 'b, 'b>(mut guard: MutexGuard<'a, Self>, t: &'b mut TestState<'_>) -> MutexGuard<'a, Self> {
...
fn monitor_outside_struct() {
    ...
    let monitor = Monitor::<TestState, TestCoordination, TestCriticalSection>::new(coordination, critical_section);
    let cnd = Condvar::new();
