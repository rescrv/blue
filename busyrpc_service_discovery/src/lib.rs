...
impl MemoryServiceDiscovery {
    ...
    fn key_and_host_for_binding(
        &self,
        binding: Binding,
    ) -> Result<(TupleKey, TupleKey, Host), SError> {
    ...
    fn check_routing_key(&self, routing_key: &[u8]) -> Result<TupleKey, SError> {
        let routing_key = TupleKey::from(routing_key);
        if self.conf.schema().lookup(&routing_key).is_err() {
            return Err(rpc_pb::not_found(
                "routing key doesn't match any hosts in the host map",
            ));
        }
        ...
        if !self.conf.schema().is_terminal(&routing_key).unwrap() {
            return Err(rpc_pb::not_found("routing key is not terminal"));
        }
        Ok(routing_key)
    }

    fn check_host(&self, host: &Host) -> Result<(), SError> {
        if host.host_id() == HostID::BOTTOM {
            Err(rpc_pb::resolve_failure(
                "host id not permitted to be bottom",
            ))
        } else if host.host_id() == HostID::TOP {
            Err(rpc_pb::resolve_failure("host id not permitted to be top"))
        } else {
            Ok(())
        }
    }
...
impl ServiceDiscovery for MemoryServiceDiscovery {
    fn register(
        ...
        _: &rpc_pb::Context,
        req: RegisterRequest,
    ) -> Result<RegisterResponse, SError> {
    ...
    ) -> Result<UnregisterResponse, SError> {
    ...
    fn resolve(&self, _: &rpc_pb::Context, req: ResolveRequest) -> Result<ResolveResponse, SError> {
        let ResolveRequest {
            routing_key,
            service_key,
            limit,
        ...
        let Some(route) = self.conf.args(std::slice::from_ref(&routing_key)).ok() else {
            ...
            return Err(rpc_pb::resolve_failure("invalid routing key"));
        };
