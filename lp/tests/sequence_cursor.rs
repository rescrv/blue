...
use lp::reference::{ReferenceBuilder, ReferenceCursor, ReferenceTable};
use lp::sequence_cursor::SequenceCursor;

mod alphabet;

fn sequence_cursor(_: &str) -> SequenceCursor<ReferenceTable, ReferenceCursor> {
    ...
    tables.push(builder.seal().unwrap());
    SequenceCursor::new(tables).unwrap()
}
