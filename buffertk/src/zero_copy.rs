...
impl<'a, B, E, V> Drop for ZeroCopy<'a, B, E, V>
where
    B: Backing,
    V: Unpackable<'a, Error=E> + 'a,
{
...
mod tests {
    ...
    fn two_slices() {
        let exp: &[u8] = &[8, 0, 1, 2, 3, 4, 5, 6, 7, 8, 8, 9, 10, 11, 12, 13, 14, 15];
        let zcu: Pin<Box<ZeroCopy<&[u8], Error, TwoSlices>>> = ZeroCopy::new(exp).unwrap();
