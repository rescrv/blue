...
fn main() {
    ...
    for i in 0..86_400i64 {
        ...
        store.push_gauge(
            &GAUGE_NEG_SIN_10.label().into(),
            &tags,
            Time::from_secs(i),
            GAUGE_NEG_SIN_10.read(),
        );
        GAUGE_NEG_SIN_60.set(0.0 - generate_sin(60, i));
        store.push_gauge(
            &GAUGE_NEG_SIN_60.label().into(),
            &tags,
            Time::from_secs(i),
            GAUGE_NEG_SIN_60.read(),
        );
        GAUGE_NEG_SIN_1200.set(0.0 - generate_sin(1200, i));
        store.push_gauge(
            &GAUGE_NEG_SIN_1200.label().into(),
            &tags,
            Time::from_secs(i),
            GAUGE_NEG_SIN_1200.read(),
        );
        GAUGE_NEG_SIN_3600.set(0.0 - generate_sin(3600, i));
        store.push_gauge(
            &GAUGE_NEG_SIN_3600.label().into(),
            &tags,
            Time::from_secs(i),
            GAUGE_NEG_SIN_3600.read(),
        );
        // Cos waves
        GAUGE_COS_10.set(generate_cos(10, i));
        store.push_gauge(
            &GAUGE_COS_10.label().into(),
            &tags,
            Time::from_secs(i),
            GAUGE_COS_10.read(),
        );
    ...
    let plot60 = Plot::new("λ = 60", Query::Gauge("waves.sin.60".into(), tags.clone()))
        .with_y_min(0.0)
        .with_y_max(1.0);
    let plot1200 = Plot::new(
        "λ = 1200",
        Query::Gauge("waves.sin.1200".into(), tags.clone()),
    )
    .with_y_min(0.0)
    .with_y_max(1.0);
    let plot3600 = Plot::new(
        "λ = 3600",
        Query::Gauge("waves.sin.3600".into(), tags.clone()),
    )
    .with_y_min(0.0)
    .with_y_max(1.0);
    ...
    let plot1200 = Plot::new(
        "λ = 1200",
        Query::Gauge("waves.cos.1200".into(), tags.clone()),
    )
    .with_y_min(0.0)
    .with_y_max(1.0);
    let plot3600 = Plot::new(
        "λ = 3600",
        Query::Gauge("waves.cos.3600".into(), tags.clone()),
    )
    .with_y_min(0.0)
    .with_y_max(1.0);
    ...
    let plot = Plot::new(
        ...
        Query::Union(vec![
            Query::Gauge("waves.sin.1200".into(), tags.clone()),
            Query::Gauge("waves.neg_sin.1200".into(), tags.clone()),
        ]),
    )
    .with_y_min(-1.0)
    .with_y_max(1.0);
    let panel3 = Panel::new("matched sin waves").with_plot(plot);
    let dash = Dashboard::new("Periodic Functions")
        .with_panel(panel1)
        .with_panel(panel2)
        .with_panel(panel3)
        .with_label_width(20);
    let qe = QueryEngine::new().with_biometrics_store(store);
    let mut rewindable = RewindWriter::from(std::io::stdout());
    for i in 0..8_640 - 360 {
        rewindable.rewind().unwrap();
        dash.plot(
            &qe,
            Window(Time::from_secs(i * 10), Time::from_secs(i * 10 + 3_600)),
            &mut rewindable,
        )
        .unwrap();
