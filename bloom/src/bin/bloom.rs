...
use bloom::{calc_keys_given_probability, calc_m_given_p_n, calc_p_given_n_m, Parameter, N, P, M, K};
...
fn main() {
    ...
    match (n, p, m, k) {
        (None, Some(p), None, None) => {
            println!("a bloom filter with false positive rate of {} will work best with {} keys", p.0, calc_keys_given_probability(p).0);
        }
        (Some(n), Some(p), None, None) => {
            println!("a bloom filter for {} items with false positive rate {} will need {} bits", n.0, p.0, calc_m_given_p_n(p, n).0);
        }
        (Some(n), None, Some(m), None) => {
            println!("a bloom filter for {} items with {} bits will have a {} false positive rate", n.0, m.0, calc_p_given_n_m(n, m).0);
        }
        (_, _, _, _) => {
