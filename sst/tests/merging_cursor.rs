...
fn merging_cursor_no_overlap(_: &str) -> MergingCursor<ReferenceCursor> {
    let mut cursors: Vec<ReferenceCursor> = Vec::new();
    ...
    cursors.push(builder.seal().unwrap().cursor());
    let mut builder = ReferenceBuilder::default();
...
fn merging_cursor_total_overlap(_: &str) -> MergingCursor<ReferenceCursor> {
    let mut cursors: Vec<ReferenceCursor> = Vec::new();
    let mut builder = ReferenceBuilder::default();
    builder.put("A".as_bytes(), 0, "a".as_bytes()).unwrap();
    builder.put("N".as_bytes(), 0, "n".as_bytes()).unwrap();
